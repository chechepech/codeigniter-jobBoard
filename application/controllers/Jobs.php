...
class Jobs extends MY_Controller {
  ...
  public function index() {
    ...
    $page_data['query'] = $this->Jobs_model->get_jobs($this->input->post('search_string',TRUE));

    if ($this->form_validation->run() == FALSE) {
      $page_data['search_string'] = array('name' => 'search_string', 'class' => 'form-control', 'id' => 'search_string', 'value' => set_value('search_string', $this->input->post('search_string',TRUE)), 'maxlength'   => '100', 'size' => '35');
  ...
  public function create() {
    ...
    if ($this->form_validation->run() == FALSE) {
      ...
      $save_data = array(
        ...
        'job_start_date' => $this->input->post('start_y',TRUE) .'-'.$this->input->post('start_m',TRUE).'-'.$this->input->post('start_d',TRUE),
        'job_rate' => $this->input->post('job_rate'),
        'job_advertiser_name' => $this->input->post('job_advertiser_name',TRUE),
        'job_advertiser_email' => $this->input->post('job_advertiser_email',TRUE),
        'job_advertiser_phone' => $this->input->post('job_advertiser_phone',TRUE),
        'job_sunset_date' => $this->input->post('sunset_y',TRUE) .'-'.$this->input->post('sunset_m',TRUE).'-'.$this->input->post('sunset_d',TRUE),
        );
  ...
  public function apply() {
    $this->form_validation->set_rules('job_id', $this->lang->line('job_title'), 'required|min_length[1]|max_length[125]|trim');
    $this->form_validation->set_rules('app_name', $this->lang->line('app_name'), 'required|min_length[1]|max_length[125]|trim');
    $this->form_validation->set_rules('app_email', $this->lang->line('app_email'), 'required|min_length[1]|max_length[125]|trim');
    $this->form_validation->set_rules('app_phone', $this->lang->line('app_phone'), 'min_length[1]|max_length[125]|trim');
    $this->form_validation->set_rules('app_cover_note', $this->lang->line('app_cover_note'), 'required|min_length[1]|max_length[3000]|trim');

    if ($this->input->post()) {
      $page_data['job_id'] = $this->input->post('job_id',TRUE);
    } else {
    ...
    if ($this->form_validation->run() == FALSE) {
      $page_data['job_id'] = array('name' => 'job_id', 'class' => 'form-control', 'id' => 'job_id', 'value' => set_value('job_id', ''), 'maxlength' => '100', 'size' => '35','required'=>'required');
      $page_data['app_name'] = array('name' => 'app_name', 'class' => 'form-control', 'id' => 'app_name', 'value' => set_value('app_name', ''), 'maxlength' => '100', 'size' => '35','required'=>'required');
      $page_data['app_email'] = array('type'=>'email','name' => 'app_email', 'class' => 'form-control', 'id' => 'app_email', 'value' => set_value('app_email', ''), 'maxlength' => '100', 'size' => '35','required'=>'required');
      $page_data['app_phone'] = array('type'=>'number','name' => 'app_phone', 'class' => 'form-control', 'id' => 'app_phone', 'value' => set_value('app_phone', ''), 'maxlength' => '100', 'size' => '35','required'=>'required');
      ...
      $body = str_replace('%app_name%', $this->input->post('app_name',TRUE), $body);
      $body = str_replace('%job_title%', $page_data['job_title'], $body);
      $body = str_replace('%app_email%', $this->input->post('app_email',TRUE), $body);
      $body = str_replace('%app_phone%', $this->input->post('app_phone',TRUE), $body);
      $body = str_replace('%app_cover_note%', $this->input->post('app_cover_note',TRUE), $body);
